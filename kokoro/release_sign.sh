...
# find the latest directory under prod/endpoints-framework-maven-plugin/gcp_ubuntu/release-build/
LAST_BUILD=$(ls prod/endpoints-framework-maven-plugin/gcp_ubuntu/release-build/ | sort -rV | head -1)

# find the jars and the pom in the latest build artifact directory
FILES=$(find `pwd`/prod/endpoints-framework-maven-plugin/gcp_ubuntu/release-build/${LAST_BUILD}/* -type f \( -iname \*.jar -o -iname \*.pom \))
