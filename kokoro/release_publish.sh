...
LAST_BUILD=$(ls prod/endpoints-framework-maven-plugin/ubuntu/release-sign/ | sort -rV | head -1)

# Finds the bundled jar file in the latest signed artifact directory.
BUNDLED_JAR_FILE=$(find `pwd`/prod/endpoints-framework-maven-plugin/ubuntu/release-sign/${LAST_BUILD}/* -type f \( -iname \*-bundle.jar \))
